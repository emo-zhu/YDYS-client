<template>
  <n-form ref="formRef" :model="model" :rules="rules" label-placement="left" label-width="98">
    <n-grid :cols="2" :x-gap="16">
      ...
      <n-form-item-gi label="姓名" path="staffName">
        <n-input v-model:value="model.staffName" placeholder="请输入姓名" />
      </n-form-item-gi>
      <n-form-item-gi label="工号" path="staffNo">
        <n-input v-model:value="model.staffNo" placeholder="请输入工号" />
      </n-form-item-gi>
      <n-form-item-gi label="所属科室" path="departmentName">
        <n-select v-model:value="model.departmentName" :options="departmentOptions" placeholder="请选择所属科室" />
      </n-form-item-gi>
...
const model = defineModel<StaffArchiveForm>({
  default: {
    archiveYear: '2025',
    archiveCode: '',
    staffName: '',
    ...
    departmentName: '',
    positionName: '',
    archiveStatus: '待完善' as StaffArchiveStatus,
    assessmentLevel: '良好' as StaffArchiveLevel,
    score: 90,
    rewardCount: 0,
    complaintCount: 0,
    highlights: '',
