<template>
  <div v-if="hasVisibleData" ref="cardRef" class="chart-card">
    ...
    <div
      ...
      :class="{ 'chart-card__body--rank': showRankPanel }"
    >
    ...
    <div
      v-if="hasAction('setting') || hasAction('download')"
      class="chart-card__tools"
    >
      <n-popover
      ...
      >
        <template #trigger>
          <button class="chart-card__tool" type="button" title="设置">
            <svg viewBox="0 0 24 24" aria-hidden="true">
              <path
                d="M19.43 12.98c.04-.32.07-.65.07-.98s-.02-.66-.07-.98l2.11-1.65a.5.5 0 0 0 .12-.64l-2-3.46a.5.5 0 0 0-.6-.22l-2.49 1a7.28 7.28 0 0 0-1.69-.98L14.5 2.42A.49.49 0 0 0 14 2h-4a.49.49 0 0 0-.49.42l-.38 2.65c-.61.24-1.18.56-1.69.98l-2.49-1a.5.5 0 0 0-.6.22l-2 3.46a.5.5 0 0 0 .12.64l2.11 1.65c-.04.32-.08.65-.08.98s.03.66.08.98l-2.11 1.65a.5.5 0 0 0-.12.64l2 3.46c.13.23.4.32.6.22l2.49-1c.51.4 1.08.73 1.69.98l.38 2.65c.04.24.25.42.49.42h4c.24 0 .45-.18.49-.42l.38-2.65c.61-.24 1.18-.56 1.69-.98l2.49 1c.23.08.48 0 .6-.22l2-3.46a.5.5 0 0 0-.12-.64l-2.11-1.65ZM12 15.5A3.5 3.5 0 1 1 12 8a3.5 3.5 0 0 1 0 7.5Z"
              />
            </svg>
          </button>
        </template>
        <div class="chart-setting">
          <div class="chart-setting__title">图表设置</div>
          <n-space vertical :size="12">
            <n-switch v-model:value="chartSetting.showLegend">
              <template #checked>显示图例</template>
              <template #unchecked>隐藏图例</template>
            </n-switch>
            <n-switch v-model:value="chartSetting.showLabel">
              <template #checked>显示数值</template>
              <template #unchecked>隐藏数值</template>
            </n-switch>
          </n-space>
        </div>
      </n-popover>
      ...
      >
        <svg viewBox="0 0 24 24" aria-hidden="true">
          <path
            d="M12 3a1 1 0 0 1 1 1v9.17l3.59-3.58A1 1 0 1 1 18 11l-5.3 5.3a1 1 0 0 1-1.4 0L6 11a1 1 0 1 1 1.41-1.41L11 13.17V4a1 1 0 0 1 1-1Zm-7 15a1 1 0 0 1 1 1v1h12v-1a1 1 0 1 1 2 0v2a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1v-2a1 1 0 0 1 1-1Z"
          />
        </svg>
      </button>
    </div>
...
const showRankPanel = computed(
  () => props.card.kind === "donut" && rankItems.value.length > 0,
);

const hasVisibleData = computed(() => {
  if (props.card.kind === "bar")
    return Boolean(props.card.bars?.some((item) => item.value !== 0));
  if (props.card.kind === "donut")
    return Boolean(props.card.donut?.some((item) => item.value !== 0));
  if (props.card.kind === "line") {
    return Boolean(
      props.card.lines?.some((line) =>
        line.values.some((value) => value !== 0),
      ),
    );
  }
  return false;
...
const chartData = computed(() => {
  if (props.card.kind === "bar") {
    return (props.card.bars || []).map((item) => ({
      item: item.name,
      count: item.value,
      label: item.label,
    }));
  ...
  if (props.card.kind === "donut") {
    return (props.card.donut || []).map((item) => ({
      item: item.name,
      count: item.value,
    }));
  }
...
const onDownload = async () => {
  const success = await chartRef.value?.downloadImage(
    props.card.title || "分析图",
  );
  if (success) {
